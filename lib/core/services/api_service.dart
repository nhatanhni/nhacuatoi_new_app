...
class ApiService {
  ...
  Future<Map<String, dynamic>> login(String username, String password) async {
    ...
    if (response.statusCode == 200 &&
      ...
      // Tự động gọi API lấy thông tin người dùng
      final userInfoResponse = await getUserInfo(accessToken);
      if (userInfoResponse['Success'] == true &&
          userInfoResponse['Data'] != null) {
        final userInfoData = userInfoResponse['Data'];
        // Gộp token vào dữ liệu user để dễ dàng lưu trữ
        userInfoData['AccessToken'] = accessToken;
        return userInfoData;
      } else {
        throw Exception(
          userInfoResponse['Message'] ?? 'Không lấy được thông tin người dùng',
        );
      }
    } else {
