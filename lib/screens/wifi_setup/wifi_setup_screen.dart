...
import 'package:iot_app/core/utils/qr_code_helper.dart';
// import 'package:permission_handler/permission_handler.dart';
import 'package:iot_app/core/utils/permission_helper.dart';
...
class _WiFiSetupScreenState extends State<WiFiSetupScreen> {
  ...
  Future<void> _requestPermissions() async {
    ...
    final currentLocationWhenInUseStatus =
        await Permission.locationWhenInUse.status;

    print('📍 Current location status: $currentLocationStatus');
    print(
      '📍 Current locationWhenInUse status: $currentLocationWhenInUseStatus',
    );
    ...
    if (Platform.isAndroid) {
      final nearbyWifiDevicesStatus = await Permission.nearbyWifiDevices
          .request();
    ...
    if (locationWhenInUseStatus != PermissionStatus.granted &&
      ...
      _showMessage(
        'Cần quyền vị trí để quét WiFi. Vui lòng cấp quyền và thử lại.',
      );
    } else {
  ...
  Widget _buildWiFiScanStep() {
    return SingleChildScrollView(
      // Wrap toàn bộ với SingleChildScrollView
      child: Column(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Card(
                              ...
                              child: ListTile(
                                ...
                                subtitle: Text(
                                  '${wifi.level} dBm • ${_getFrequencyBand(wifi.frequency)}',
                                ),
                                trailing: Row(
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                if (selectedWiFi!.capabilities.contains('WPA') ||
                    selectedWiFi!.capabilities.contains('WEP'))
  ...
  Widget _buildDeviceSetupStep() {
    return Column(
      children: [
        if (isSettingUp) const CircularProgressIndicator(),
        const SizedBox(height: 16),
        Text(
          statusMessage,
          style: TextStyle(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Text(
                        '${'*' * passwordController.text.length} (${passwordController.text.length} ký tự)',
                      ),
                    ],
                  ),
                  const SizedBox(height: 6),
        ...
        if (selectedWiFi != null && passwordController.text.isNotEmpty) ...[
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(
                isSettingUp ? 'Đang Cài Đặt...' : 'Giả Lập Browser & Submit',
              ),
              style: ElevatedButton.styleFrom(
    ...
    try {
      ...
      if (qrData.startsWith('WIFI:')) {
        ...
        if (scannedSSID != null) {
          setState(() {
            statusMessage =
                'Đã quét thành công WiFi: $scannedSSID\nĐang kết nối vào thiết bị...';
          });
  ...
  Future<void> _connectToScannedWiFi() async {
    ...
    try {
      ...
      if (connected) {
        setState(() {
          statusMessage =
              'Đã kết nối vào thiết bị IoT thành công!\nBắt đầu quét WiFi để cài đặt...';
          currentStep = 2;
        });
        ...
        setState(() {
          statusMessage =
              'Kết nối vào thiết bị IoT thất bại.\nVui lòng kiểm tra lại QR code.';
        });
      }
    } catch (e) {
      setState(() {
    ...
    try {
      ...
      if (locationStatus != PermissionStatus.granted &&
        ...
        if (newLocationStatus != PermissionStatus.granted) {
          throw Exception(
            'Cần quyền vị trí để quét WiFi. Vui lòng cấp quyền trong Cài đặt.',
          );
        }
      }
      ...
      if (canStartScan != CanStartScan.yes) {
        switch (canStartScan) {
          ...
          case CanStartScan.noLocationPermissionDenied:
            throw Exception(
              'Quyền vị trí bị từ chối. Vui lòng cấp quyền trong Cài đặt.',
            );
          case CanStartScan.noLocationServiceDisabled:
      ...
      for (var wifi in results.take(10)) {
        // Chỉ in 10 WiFi đầu để không spam log
  ...
  Future<void> _testDeviceConnection() async {
    ...
    try {
      ...
      print(
        '📡 Test response body: ${response.body.substring(0, response.body.length > 200 ? 200 : response.body.length)}...',
      );
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          statusMessage =
              '⚠️ Thiết bị phản hồi nhưng có lỗi\nHTTP Status: ${response.statusCode}';
        });
      }
    } on TimeoutException {
      setState(() {
...
⚠️ Lỗi chi tiết: $e''';
  ...
  Future<void> _setupDeviceWiFi() async {
    ...
    try {
      ...
      final url =
          'http://192.168.4.1/setting?ssid=${Uri.encodeComponent(ssid)}&pass=${Uri.encodeComponent(password)}';
